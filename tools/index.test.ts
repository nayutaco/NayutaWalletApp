...
test('[NG] BIP21 - bitcoin:bc(testnet)', async () => {
  ...
  expect(async () => await qrResolver('bitcoin:bc1qdc6r8l3p28czx5zhvmq68afy9n7g85qkjarutv?amount=0.1')).rejects.toThrow('not correct bip21');
});
...
test('[NG] BIP21 - bitcoin:tb+amount(testnet)', async () => {
  ...
  expect(async () => await qrResolver('tb1qrpyguxynsvl4t27dunmwxv9cr8fplvfyp065yu?amount=-0.1')).rejects.toThrow('not correct bip21');
});
...
test('[NG] not BIP21 - testnet(bc)', async () => {
  ...
  expect(async () => await qrResolver('bc1qdc6r8l3p28czx5zhvmq68afy9n7g85qkjarutv')).rejects.toThrow('not testnet address');
});
...
test('[NG] not BIP21 - mainnet(tb)', async () => {
  ...
  expect(async () => await qrResolver('TB1QHUU23ETMT3QVENUV7RCYG38CPHRYN9EVG92YND')).rejects.toThrow('not mainnet address');
});
...
test('[NG] not BIP21 - testnet(1)', async () => {
  ...
  expect(async () => await qrResolver('1F1tAaz5x1HUXrCNLbtMDqcw6o5GNn4xqX')).rejects.toThrow('not testnet address');
});
...
test('[NG] not BIP21 - testnet(3)', async () => {
  ...
  expect(async () => await qrResolver('37NFX8KWAQbaodUG6pE1hNUH1dXgkpzbyZ')).rejects.toThrow('not testnet address');
});
...
test('[NG] not BIP21 - mainnet(m)', async () => {
  ...
  expect(async () => await qrResolver('mvH6CfDe9hoorbG1GCCe3ocPdvx8qdKszn')).rejects.toThrow('not mainnet address');
});
...
test('[NG] lninvoice(mainnet(bc))', async () => {
  ...
  ).rejects.toThrow('not usable invoice');
});
...
//       ),
//   ).rejects.toThrow('not correct LNURL');
// });
...
test('[NG] lnurl scheme(not correct)', async () => {
  expect(async () => await qrResolver('https://service.com/giftcard/redeem?id=123&lightning=lnurl')).rejects.toThrow('not correct LNURL');
});

test('[NG] lnurl scheme(not exist)', async () => {
  expect(async () => await qrResolver('https://service.com/giftcard/redeem')).rejects.toThrow('not exist LNURL');
});
...
test('[NG] unknown - lndconnect', async () => {
  ...
  ).rejects.toThrow('lndconnect is not supported');
});

test('[NG] unknown', async () => {
  expect(async () => await qrResolver('')).rejects.toThrow('unknown error');
});
