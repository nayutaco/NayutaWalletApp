...
import {View, StyleSheet, Text, ActivityIndicator, AppState, SafeAreaView} from 'react-native';
...
export default function QrScanner() {
  ...
  useEffect(() => {
    if (alertIndex === 0) {
      node !== null && node.current.reactivate();
    }
  }, [alertIndex]);
  ...
  const onRead = async ({data}: {data: string}) => {
    setLoading(true);
    setReadError('');
      ...
      <View>
        ...
        <View style={styles.cameraView}>
          <QRCodeScanner
            ...
            permissionDialogTitle={t('qrscanner:permissionTitle')}
            permissionDialogMessage={t('qrscanner:permissionMsg')}
            buttonPositive={t('ok')}
            showMarker={true}
