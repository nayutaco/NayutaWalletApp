...
export const HomeScreen = () => {
  ...
  const convertedFiatCap = useSatToCurrency(store.fiat, constants.otfInvoiceCap.toString());
  ...
  const receiveCheckOnTheFly = async (amount: Satoshi): Promise<boolean> => {
    ...
    if (amountNum > constants.otfInvoiceCap) {
      const convertedCryptoCap =
        store.crypto === 'sat'
          ? convertStringNumber(constants.otfInvoiceCap, store.digitDecimalRule)
          : Satoshi.fromSat(constants.otfInvoiceCap).toBTC().toString();
      setError(
        t('home:excessOtfCap', {
