...
import {duringCopying, maxCapacitySat} from 'tools/constants';
...
export default function SwapReceiveScreen() {
  ...
  const [submarineSwapCap, setSubmarineSwapCap] = useState<Satoshi | undefined>();
  ...
  useEffect(() => {
    ...
    const capacityBalanceDiff = Satoshi.fromSat(maxCapacitySat).sub(balance.offChain.local);
    const cap = capacityBalanceDiff.toNumber() > params.minFee ? capacityBalanceDiff : Satoshi.fromSat(0);
    setSubmarineSwapCap(cap);
  }, [status, params.minFee]);
